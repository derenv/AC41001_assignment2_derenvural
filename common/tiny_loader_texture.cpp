...
void TinyObjLoader::load_obj(string inputfile, bool debugPrint, bool smoothShade)
{
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) 
		{
			...
			for (size_t v = 0; v < fv; v++)
			{
				...
				if (idx.texcoord_index != -1) {
					...
					//if not present enter placeholder
					pTextureCoords[ind * 2 + 0] = 0;//x
					pTextureCoords[ind * 2 + 1] = 0;//y
				}
...
{
	// Replicate array of colours
	std::vector<tinyobj::real_t> pColourValues(numVertices * 3);
	for (int i = 0; i < numVertices*3; i+=3)
	{
		pColourValues[i] = c.x;
		pColourValues[i+1] = c.y;
		pColourValues[i+2] = c.z;
	}

	// Copy the colour data into OpenGL buffers
	glGenBuffers(1, &colourBufferObject);
	glBindBuffer(GL_ARRAY_BUFFER, colourBufferObject);
	glBufferData(GL_ARRAY_BUFFER, pColourValues.size() * sizeof(tinyobj::real_t), &pColourValues.front(), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
}
