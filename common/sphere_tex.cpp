 ...
 */
void Sphere::overrideColour(glm::vec4 c)
{
	glm::vec4* pColours = new glm::vec4[numspherevertices];
	...
	glBindBuffer(GL_ARRAY_BUFFER, sphereColours);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec4) * numspherevertices, pColours, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
}
