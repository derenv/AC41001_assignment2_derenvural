...
void Cube::makeCube()
{
	...
	GLfloat texcoords[] =
	{
		...
		// Face 1
		.3f, 0.f,   0.f, 0.f,   .3f, .3f,
		0.f, 0.f,   0.f, .3f,   .3f, .3f,
		...
		// Face 4
		0.f, 0.f,   .3f, 0.f,   .3f, .3f,
		.3f, .3f,   0.f, .3f,   0.f, 0.f,

		// Face 5
		0.f, .3f,   .3f, .3f,   .3f, 0.f,
		.3f, 0.f,   0.f, 0.f,   0.f, .3f
	};
 ...
 */
void Cube::overrideColour(glm::vec4 c)
{
	glm::vec4* pColours = new glm::vec4[numvertices];
	...
	glBindBuffer(GL_ARRAY_BUFFER, colourObject);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec4) * numvertices, pColours, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
}
